    ...
    for (int run = 0; run < n_runs; run++)
    {
        ...
        for (int iter = 0; iter < n_it; iter++)
        {
            ...
            for (int i = 0; i < N; i++)
            {
                ...
                v[i] = v_i + dt * (1.0f-theta)  * (grad_f_i - v_i);
                x[i] = x_i - dt * eta * v_i + gamma * dist(generator); 
...
class BayesianOptimization : public bayesopt::ContinuousModel {
    public:
        ...
        double evaluateSample( const vectord &Xi ) {
            ...
            this->alpha = Xi(0); this->beta = Xi(1); this->theta = Xi(2); this->eta = Xi(3); this->gamma = Xi(4); this->dt = Xi(5); 
...
int main(int argc, char* argv[])
{
    ...
    filename << "BayesianOptimization_Algorithmic_Momentum_Sigmoid_Sequential_CACdisabled_" << "G" << problem << ".txt";
...
////////////////////////////////////////////////////////////// CONSTANTS //////////////////////////////////////////////////////////////////////////
    ...
    lb(0) = opt.alpha_min + 0.000000001; lb(1) = opt.beta_min + 0.000000001; lb(2) = opt.theta_min + 0.000000001; lb(3) = opt.eta_min + 0.000000001; lb(4) = opt.gamma_min; lb(5) = opt.dt_min; 
    ub(0) = opt.alpha_max - 0.000000001; ub(1) = opt.beta_max - 0.000000001; ub(2) = opt.theta_max - 0.000000001; ub(3) = opt.eta_max - 0.000000001; ub(4) = opt.gamma_max; ub(5) = opt.dt_max; 
