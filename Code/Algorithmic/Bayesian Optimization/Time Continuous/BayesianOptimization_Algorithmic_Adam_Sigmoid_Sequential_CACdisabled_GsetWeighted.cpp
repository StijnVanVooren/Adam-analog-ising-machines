...
void loadJ(std::vector<std::vector<std::array<int, 2>>>& J, const int N, const int problem, double &E_target, double TTS_treshhold) 
{
    ...
    while (std::getline(file, line)) {
        ...
        if (j - 1 < N) {
            J[j - 1].push_back({ i - 1 ,value });
            J[i - 1].push_back({ j - 1, value });
        }
    }
    ...
    E_target = 0.; 
    for (int i = 0; i < N; i++) {
        for(auto& element : J[i]){
            E_target += static_cast<double>(element[1]);
        }        
    }
    E_target *= 0.5;
...
std::array<double, 7> Ising_Machine(const double alpha, const double beta, const double theta, const double beta1, const double beta2, const double eta, const double gamma, const double dt, const double alpha_CAC, const double a_CAC, const double delta_CAC, const double rho_CAC, const double ksi_CAC, const double Gamma_CAC, const int n_x_CAC, const int n_e_CAC, const int DT_CAC, const int n_runs, const int n_it, const int N, const int problem){
    ...
    // Initialization  
    // Coupling matrix
    std::vector<std::vector<std::array<int, 2>>> J(N);
    double E_target = 0.;
    ...
    for (int run = 0; run < n_runs; run++)
    {
        ...
        {
            double adaptive_learning_rate = eta*std::sqrt(1.0f-std::pow(beta2,t))/(1.0f-std::pow(beta1,t));
            ...
            for (int i = 0; i < N; i++)
            {
                ...
                v[i] = v_i + dt * (1.0f-beta1)  * (grad_f_i - v_i);
                w[i] = w_i + dt * (1.0f-beta2) * (std::pow(grad_f_i, 2) - w_i);
                x[i] = x_i - dt * adaptive_learning_rate * v_i / (std::sqrt(w_i) + 0.00000001) + gamma * dist(generator); 
...
class BayesianOptimization : public bayesopt::ContinuousModel {
    public:
        ...
        double evaluateSample( const vectord &Xi ) {
            ...
            this->alpha = Xi(0); this->beta = Xi(1); this->beta1 = Xi(2); this->beta2 = Xi(3); this->eta = Xi(4); this->gamma = Xi(5); this->dt = Xi(6); 
    ...
    public:
        ...
        double beta1_min = 0., beta1_max = 1.;
        double beta2_min = 0., beta2_max = 1.;
...
int main(int argc, char* argv[])
{
    ...
    // Problem constants
    const std::vector<int> N_s = { 800,800,800,800,800,800,800,800,800,800,800,800,2000,2000,2000,2000,2000,2000,2000,2000,2000,2000,2000,2000 };
    const std::vector<int> problems = { 6,7,8,9,10,11,12,13,18,19,20,21,27,28,29,30,31,32,33,34,39,40,41,42 };
    const int N = N_s[rank];
    const int problem = problems[rank];
    // Generate a unique filename for each process
    std::ostringstream filename;
    filename << "BayesianOptimization_Algorithmic_Adam_Sigmoid_Sequential_CACdisabled_" << "G" << problem << ".txt";
    // Extract the string from the ostringstream
    std::string path = "/dodrio/scratch/projects/starting_2026_056/results/run2/";
...
////////////////////////////////////////////////////////////// CONSTANTS //////////////////////////////////////////////////////////////////////////
    ...
    lb(0) = opt.alpha_min + 0.000000001; lb(1) = opt.beta_min + 0.000000001; lb(2) = opt.beta1_min + 0.000000001; lb(3) = opt.beta2_min + 0.000000001; lb(4) = opt.eta_min + 0.000000001; lb(5) = opt.gamma_min; lb(6) = opt.dt_min; 
    ub(0) = opt.alpha_max - 0.000000001; ub(1) = opt.beta_max - 0.000000001; ub(2) = opt.beta1_max - 0.000000001; ub(3) = opt.beta2_max - 0.000000001; ub(4) = opt.eta_max - 0.000000001; ub(5) = opt.gamma_max; ub(6) = opt.dt_max; 
