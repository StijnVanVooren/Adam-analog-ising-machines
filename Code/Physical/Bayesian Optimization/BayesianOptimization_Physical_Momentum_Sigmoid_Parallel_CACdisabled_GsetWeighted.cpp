...
{
    std::string path_Gset = "/dodrio/scratch/projects/2025_009/Stijn/problem_sets/Gset/";
...
#pragma omp parallel for reduction(+:T_a_Euler, T_a_CPU, SR, SR_tr) reduction(min:E_min) schedule(dynamic) 
    for (int run = 0; run < n_runs; run++)
    {
        ...
        std::vector<double> x_new(N), v_new(N);
        double x_i, x_bin_i, v_i; 
        ...
        for (int iter = 0; iter < n_it; iter++)
        {
            ...
            for (int i = 0; i < N; i++)
            {
                ...
                // Update m,v,x,CAC:
                v_new[i] = v_i + dt * (1.0f-theta)  * (grad_f_i - v_i);
                x_new[i] = x_i - dt * eta * v_i + gamma * dist(generator); 
...
class BayesianOptimization : public bayesopt::ContinuousModel {
    public:
        ...
        double evaluateSample( const vectord &Xi ) {
            ...
            this->alpha = Xi(0); this->beta = Xi(1); this->theta = Xi(2); this->gamma = Xi(3); 
...
int main(int argc, char* argv[])
{
    ...
    filename << "BayesianOptimization_Physical_Momentum_Sigmoid_Parallel_CACdisabled_" << "G" << problem << ".txt";
    // Extract the string from the ostringstream
    std::string path = "/dodrio/scratch/projects/2025_009/Stijn/results/run1/";
...
// BayesOpt parameters  
    ...
    params.n_iterations = 700;
    params.noise = std::pow(10.,-3);
    params.verbose_level = 5;
    const size_t bayesopt_dim_ = 4;
...
////////////////////////////////////////////////////////////// CONSTANTS //////////////////////////////////////////////////////////////////////////
    ...
    lb(0) = opt.alpha_min + 0.000000001; lb(1) = opt.beta_min + 0.000000001; lb(2) = opt.theta_min + 0.000000001; lb(3) = opt.gamma_min; 
    ub(0) = opt.alpha_max - 0.000000001; ub(1) = opt.beta_max - 0.000000001; ub(2) = opt.theta_max - 0.000000001; ub(3) = opt.gamma_max; 
