    ...
    for (int run = 0; run < n_runs; run++)
    {
        ...
        for (int iter = 0; iter < n_it; iter++)
        {
            ...
            for (int i = 0; i < N; i++)
            {
                ...
                x_new[i] = x_i - dt * adaptive_learning_rate * v_i / (std::sqrt(w_i) + 0.00000001) + gamma * dist(generator); 
...
int main(int argc, char* argv[])
{
    ...
    filename << "BayesianOptimization_Physical_Adam_Sigmoid_Parallel_CACdisabled_" << "G" << problem << ".txt";
